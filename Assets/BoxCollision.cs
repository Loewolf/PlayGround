...
*/
    ...
    public static float ProjVector3(Vector3 v, Vector3 a)
    {
    ...
    public static Vector3 ProjAxis(Vector3[] a, Vector3[] b, List<Vector3> Axis)
    {
        ...
        {
            //проекции куба a
            float[] max_a = new float[2];
            max_a[0] = 0;
            max_a[1] = ProjVector3(a[0], Axis[j]);
            float[] min_a = new float[2];
            min_a[0] = 0;
            min_a[1] = ProjVector3(a[0], Axis[j]);
            ...
            max_b[0] = 0;
            max_b[1] = ProjVector3(b[0], Axis[j]);
            float[] min_b = new float[2];
            min_b[0] = 0;
            min_b[1] = ProjVector3(b[0], Axis[j]);
            for (int i = 0; i < b.Length; i++)
            {
